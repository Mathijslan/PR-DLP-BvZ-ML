...
def add_gaussian_noise_to_audios(audios, train_size, SNR_db, p=0.5):
    ...
    nb_samples = 3 * 44100
    print("Adding gaussian noise to audios with SNR of %sdB with probability of %s, nb_samples %s" % (
    SNR_db, p, nb_samples))
...
def audio_augmentation(xtrain, ytrain, Fs, gn_db, p=1):
    rs = RandomState(1)
    proba = rs.uniform(low=0, high=1, size=len(xtrain))

    augment = Compose([AddGaussianNoise(gn_db, gn_db, p=1),  # 20db
