...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, inception=False):
    ...
    np.save(arr=torch.tensor(train_acc_history).detach().cpu().numpy(), file="train_acc_history")
    np.save(arr=torch.tensor(val_acc_history).detach().cpu().numpy(), file="val_acc_history")

    np.save(arr=torch.tensor(train_loss_history).detach().cpu().numpy(), file="train_loss_history")
    np.save(arr=torch.tensor(val_loss_history).detach().cpu().numpy(), file="val_loss_history")
